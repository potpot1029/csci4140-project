// adapted from https://github.com/logancyang/obsidian-copilot/blob/master/src/vectorDBManager.ts
...
class VectorDBManager {
  public static getDocumentHash(document: string): string {
  ...
  public static async getMemoryVectorStore(
    ...
    console.log('Memory Vectors:', memoryVectors);
    const memoryVectorStore = new MemoryVectorStore(embeddingsAPI);
    await memoryVectorStore.addVectors(embeddingsArray, documentsArray);
    return memoryVectorStore;
  }
  ...
  public static async setMemoryVectors(
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const existingDoc = await db.get(docHash).catch((err: any) => console.error(err));
  ...
  public static async indexFile(
    ...
    const splitDocument = await textSplitter.createDocuments([noteFile.content], [], {
      chunkHeader: '[[' + noteFile.metadata.title + ']]' + '\n\n---\n\n',
    ...
    const memoryVectors = docVectors.map((vector, i) => ({
    ...
    try {
      // fetch the existing document, if it exists
      const existingDoc = await db.get(docHash).catch(err => console.log(err));
